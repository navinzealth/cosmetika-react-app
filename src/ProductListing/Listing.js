...
function Listing() {
 ...
 const [sortSubcat, setSortSubcat]=useState(0)
  /**-sorting-data-- */
...
const fetchcatPro = async(e)=>{
  const fetchcatPro1 = await fetch(`https://cosmetikaa.com/manager/api/logic.php?action=productsByCategory&user_id=${initVal.userID}&cat_slug=${catName}&subcat_slug=${subcatName ? subcatName : 0}&sortby=${sortProduct}&brand=${sortbrand}&subcat=${sortSubcat}&key=${sortPrice == '' ? 0 : sortPrice}`)
...
const cartproID = initVal.cartData.map((item3)=>{return item3.pro_id})
 ...
 /**-srot-hight and low products */
   const sortPopular =()=>{setData(data + 1)}
 /**-srot-hight and low products */
 /*--sort-product-price-range-- */
 const sortPriceProduct =(e)=>{
    var value = e.target.value;
    var arr = sortPrice
     arr = arr.filter(function(item) {return item !== value})
    // console.log(arr)
   if(sortPrice.includes(e.target.value)){
            setSortPrice(arr);sortPopular();
                     }
   else{setSortPrice([ e.target.value, ...sortPrice]);sortPopular();}
 }
  /*--sort-product-price-range-- */
...
<section className="listing-banner">
    <div className="container">
     <div className="row"><div className="col-md-12"><div className="listing-banner-img"><img src={bannerHairCare} /></div></div>
     </div>
    </div>
</section>
           ...
           <div className="close-listing-filter-box hidden-lg hidden-md"><i className="fa fa-close"></i></div>
            ...
            <div className="listing-product-by-sort">
                ...
                <div className="listing-product-by-sort-options">
                    <label className="con">Popularity<input type="radio" checked={sortProduct === "3"} name="radio" value="3" onChange={(e)=>{setSortProduct(e.target.value);sortPopular();}}/><span className="checkmark"></span></label>

                    <label className="con">Price - Low to High<input type="radio" checked={sortProduct === "1"} name="radio" value="1" onChange={(e)=>{setSortProduct(e.target.value);sortPopular();}}/><span className="checkmark"></span></label>

                    <label className="con">Price - High to Low<input type="radio" checked={sortProduct === "2"} name="radio" value="2" onChange={(e)=>{setSortProduct(e.target.value);sortPopular();}}/><span className="checkmark"></span></label>

                    <label className="con">Newest First<input type="radio" checked={sortProduct === "4"} name="radio" value="4" onChange={(e)=>{setSortProduct(e.target.value);sortPopular();}}/><span className="checkmark"></span></label>
                </div>
             </div>
                     {/* --sort-by-value-- */}
            ...
            <div className="listin-product-filter-list">
             ...
             <div className="listing-product-filter-list-options" style={{'display':'block'}}>
               {
                 price.map((item)=>(
                  <label className="chek-1" key={item.key}><input type="checkbox" value={item.key}  onChange={sortPriceProduct}
                  checked={sortPrice.includes(item.key) }/>  
                  <span className="checkmark1" ></span>{item.price}</label>
                 ))
               }
             </div>sortPriceProduct
            </div>
                        {/* --filter-by-price-- */}
                         {/* --filter-by-brand-- */}
            <div className="listin-product-filter-list">
          ...
          <div className="row">
            ...
            {/* <!--------------------------------------------------product-------------------------------------------> */}
               ...
               <div className="col-md-4">   
                  { showloadbtn ? <div onClick={showMoreItems} className="loadmore">Load More</div> : <div></div>}
              </div>
   </div>
